# Operations guide: test, fix, and document

## Mission

Maintain this deployment repository for three supported modes:
...
- Do not introduce floating image tags into production examples. The default values still use some upstream-style floating tags for compatibility, but production site values must pin versions or digests.
...
2. HPA, then render/apply `values/addons/hpa-autoscaling.yaml`, confirm metrics-server is healthy, and observe API/UI replica behavior under load.
3. KEDA, then render/apply a KEDA-enabled OntoPortal overlay, run `make keda-check NAMESPACE=<namespace> RELEASE=<release>`, and observe ScaledObject status under load.
4. VPA, then render/apply `values/addons/vpa-recommendations.yaml`, run `make vpa-check NAMESPACE=<namespace> RELEASE=<release>`, review recommendations, and only then decide whether any workload should use a mutating VPA mode.
5. kube-prometheus-stack, then OntoPortal ServiceMonitor discovery.
6. Loki, then Grafana Alloy log ingestion from OntoPortal pods.
7. Trivy Operator, then vulnerability/config reports in the OntoPortal namespace.
8. External Secrets Operator, then an `existingSecret`-based OntoPortal install.
9. Velero or another backup system, then a real restore drill into a new namespace or cluster.
10. Kyverno or another admission policy engine, then policy dry-run/audit before enforce mode.
11. Wazuh, if required, using the official Wazuh Kubernetes deployment and certificate workflow rather than embedding it in the OntoPortal chart.
12. SonarQube, if code quality scanning is required, wired to the source repositories and CI rather than deployed as part of the OntoPortal release.
...
- Shared PVC use limits horizontal scaling. Do not enable API/UI HPA, KEDA, or replicas greater than one until shared data is moved to RWX storage or externalized. Keep VPA in `Off` mode during initial right-sizing to avoid surprise pod evictions.
...
## Definition of done for repository hardening

Repository hardening is complete only when:

- `make validate`, `make validate-generated`, `make validate-ui-tests`, Docker-backed `make compose-config`, and Terraform validation complete successfully.
- Helm renders all supported profiles, including HPA, KEDA, and VPA overlays.
...
- clean OntoPortal starts on disposable k3s and Playwright UI smoke tests pass.
